...
export async function createPetugas(req, res) {
    try {
        ...
        if (!nama_petugas || !username_petugas || !password_petugas || !email_petugas || !no_hp_petugas) {
            return res.status(400).json({ message: 'Semua field wajib diisi' });
        }
        if (!EMAIL_REGEX.test(email_petugas)) {
...
export async function updatePetugas(req, res) {
    try {
        ...
        if (!nama_petugas || !username_petugas || !email_petugas || !no_hp_petugas) {
